...
import saveAIInsights from '@salesforce/apex/LadminAIAppointmentTimezoneService.saveAIInsights';
import saveCustomerNotifications from '@salesforce/apex/LadminAIAppointmentTimezoneService.saveCustomerNotifications';
export default class LadminAiAppointmentSettings extends LightningElement {
    options=[];businessTimezone;userTimezone;internalNotificationsEnabled=false;customerNotificationsEnabled=false;aiInsightsEnabled=false;loading=true;saving=false;error;
    connectedCallback(){this.load();}
    async load(){this.loading=true;this.error=null;try{const data=await getSettings();this.options=data.options||[];this.businessTimezone=data.businessTimezone;this.userTimezone=data.userTimezone;this.internalNotificationsEnabled=data.internalNotificationsEnabled===true;this.customerNotificationsEnabled=data.customerNotificationsEnabled===true;this.aiInsightsEnabled=data.aiInsightsEnabled===true;}catch(e){this.error=this.message(e,'Appointment settings could not be loaded.');}finally{this.loading=false;}}
    ...
    changeCustomerNotifications(e){this.customerNotificationsEnabled=e.target.checked;}
    changeAI(e){this.aiInsightsEnabled=e.target.checked;}
    ...
    async saveCustomerNotifications(){this.saving=true;this.error=null;try{await saveCustomerNotifications({enabled:this.customerNotificationsEnabled});this.toast('Customer notification update started','Customer confirmations and reminders are being configured.');}catch(e){this.error=this.message(e,'The customer notification setting could not be updated.');}finally{this.saving=false;}}
    async saveAI(){this.saving=true;this.error=null;try{await saveAIInsights({enabled:this.aiInsightsEnabled});this.toast('AI setting update started','Salesforce is applying the AI Smart Insights setting. Refresh after the update completes.');}catch(e){this.error=this.message(e,'The AI Smart Insights setting could not be updated.');}finally{this.saving=false;}}
