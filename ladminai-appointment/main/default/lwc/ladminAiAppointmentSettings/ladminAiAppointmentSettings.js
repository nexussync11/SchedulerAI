...
export default class LadminAiAppointmentSettings extends LightningElement {
    options=[];businessTimezone;userTimezone;internalNotificationsEnabled=false;aiInsightsEnabled=false;loading=true;saving=false;error;
    connectedCallback(){this.load();}
    async load(){this.loading=true;this.error=null;try{const data=await getSettings();this.options=data.options||[];this.businessTimezone=data.businessTimezone;this.userTimezone=data.userTimezone;this.internalNotificationsEnabled=data.internalNotificationsEnabled===true;this.aiInsightsEnabled=data.aiInsightsEnabled===true;}catch(e){this.error=this.message(e,'Appointment settings could not be loaded.');}finally{this.loading=false;}}
